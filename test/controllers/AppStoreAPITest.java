...
public class AppStoreAPITest {
    ...
    class CRUDMethods {
        ...
        void deletingAnAppThatExistsDeletesAndReturnsDeletedObject() {
            ...
            assertEquals(edAppAboveBoundary, appStore.deleteAppByIndex(4));
        ...
        void deletingAnAppThatDoesNotExistReturnsNull() {
            ...
            assertNull(appStore.deleteAppByIndex(10));
        }
        ...
        void getAppByNameReturnsAppWhenNameIsValid() {
            ...
            assertEquals(edAppAboveBoundary, appStore.getAppByName("EV3"));
        }

        @Test
        void getAppByNameReturnsNullWhenNameIsInvalid() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertNull(emptyAppStore.getAppByName("Fortnite"));
            assertEquals(7, appStore.numberOfApps());
            assertNull(appStore.getAppByName("Cod"));
            assertNull(appStore.getAppByName("BF"));
        }
        ...
        void getAppByIndexReturnsNullWhenIndexIsInvalid() {
            ...
            assertNull(appStore.getAppByIndex(25));
        }
    }
    ...
    class ListingMethods {
        ...
        @Test
        void listSummaryOfAllAppsReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listSummaryOfAllApps().toLowerCase().contains("no apps"));
        }

        @Test
        void listSummaryOfAllAppsReturnsAppsStoredWhenArrayListHasAppsStored() {
            assertEquals(7, appStore.numberOfApps());
            String apps = appStore.listSummaryOfAllApps();

            assertTrue(apps.contains("WeDo" + "(V"));
            assertTrue(apps.contains(") by "));
            assertTrue(apps.contains("€"));
            assertTrue(apps.contains("Rating: "));
        }

        @Test
        void listAllGameAppsReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllGameApps().toLowerCase().contains("no apps"));
        }

        @Test
        void listAllGameAppsReturnsAppsStoredWhenArrayListHasAppsStored() {
            assertEquals(7, appStore.numberOfApps());
            String apps = appStore.listAllGameApps();

            assertTrue(apps.contains("Is this multiplayer: "));
            assertTrue(apps.contains("EA"));
            assertTrue(apps.contains(" (Version "));
            assertTrue(apps.contains("Cost: "));
        }

        @Test
        void listAllEducationAppsReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllEducationApps().toLowerCase().contains("no apps"));
        }

        @Test
        void listAllEducationAppsReturnsAppsStoredWhenArrayListHasAppsStored() {
            assertEquals(7, appStore.numberOfApps());
            String apps = appStore.listAllEducationApps();

            assertTrue(apps.contains(" Level: "));
            assertTrue(apps.contains("Ratings ("));
            assertTrue(apps.contains(" (Version "));
            assertTrue(apps.contains("MB"));
        }

        @Test
        void listAllProductivityAppsReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllProductivityApps().toLowerCase().contains("no apps"));
        }

        @Test
        void listAllProductivityAppsReturnsAppsStoredWhenArrayListHasAppsStored() {
            assertEquals(7, appStore.numberOfApps());
        ...
        @Test
        void listAllAppsByNameReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllAppsByName("WeDo").toLowerCase().contains("no apps"));
        }

        @Test
        void listAllAppsByNameReturnsAppsWithKeywordInThem() {
            assertEquals(7, appStore.numberOfApps());
            String apps = appStore.listAllAppsByName("e");

            assertTrue(apps.contains("WeDo"));
            assertTrue(apps.contains("Empires"));
            assertTrue(apps.contains("NoteKeeper"));
            assertTrue(apps.contains("EV3"));
            assertFalse(apps.contains("Outlook"));
        }

        @Test
        void listAllAppsByDeveloperReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllAppsByChosenDeveloper(developerLego).toLowerCase().contains("no apps"));
        }

        @Test
        void listAllAppsByDeveloperReturnsAppsFromDeveloper() {
            assertEquals(7, appStore.numberOfApps());
            appStore.addApp(setupGameAppWithRating(5, 4));
            String apps = appStore.listAllAppsByChosenDeveloper(developerEAGames);

            assertTrue(apps.contains("MazeRunner"));
        }

        @Test
        void listAllAppsAboveOrEqualAGivenStarRatingReturnsNoAppsStoredWhenArrayListIsEmpty() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertTrue(emptyAppStore.listAllAppsAboveOrEqualAGivenStarRating(3).toLowerCase().contains("no apps"));
        }

        @Test
        void listAllAppsAboveOrEqualAGivenStarRatingReturnsAppsEqualOrAboveRating() {
            assertEquals(7, appStore.numberOfApps());
            appStore.addApp(setupGameAppWithRating(5, 4));
            appStore.addApp(setupProductivityAppWithRating(2, 3));
            appStore.addApp(setupEducationAppWithRating(3, 3));
            String apps = appStore.listAllAppsAboveOrEqualAGivenStarRating(3);
            assertTrue(apps.contains("MazeRunner"));
            assertTrue(apps.contains("WeDo"));
            assertFalse(apps.contains("Evernote"));
    ...
    class ReportingMethods {
        ...
        @Test
        void numberOfAppsByChosenDeveloperReturnsCorrectNumber() {
            assertEquals(7, appStore.numberOfApps());
            appStore.addApp(setupGameAppWithRating(2, 1));
            appStore.addApp(setupGameAppWithRating(3, 1));
            appStore.addApp(setupGameAppWithRating(3, 1));
            appStore.addApp(setupGameAppWithRating(3, 1));
            appStore.addApp(setupGameAppWithRating(3, 1));
            assertEquals(6, appStore.numberOfAppsByChosenDeveloper(developerEAGames));
        }

        @Test
        void randomAppReturnsNoAppsWhenNoAppsExist() {
            assertEquals(0, emptyAppStore.numberOfApps());
            assertNull(emptyAppStore.randomApp());
        }

        @Test
        void randomAppReturnsAValidNumber() {
            assertEquals(7, appStore.numberOfApps());
        }

        @Test
        void isValidAppNameReturnsTrueWhenNameIsValid() {
            assertEquals(7, appStore.numberOfApps());
            appStore.addApp(setupGameAppWithRating(4, 4));
            assertTrue(appStore.isValidAppName("MazeRunner"));
        }

        @Test
        void isValidAppNameReturnsFalseWhenNameIsInvalid() {
            assertEquals(7, appStore.numberOfApps());
            assertFalse(appStore.isValidAppName("360 Hit Squad"));
        }
